...
pub struct List<'info> {
    ...
    #[account(
        ...
        seeds::program = metadata_program.key(), // this ensures that the derived PDA is not derived with the current program key (default seeds_program = crate::ID)
        bump,
...
impl<'info> List<'info> {
    ...
    pub fn deposit_nft(&mut self) -> Result<()> {
        ...
        transfer_checked(cpi_ctx, 1, 0)?; // NFT doesn't have decimals
